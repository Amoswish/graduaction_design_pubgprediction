...
class data_process:
    ...
    def preprocess(self):
        ...
        print(df_input.tail())
        df_input_converge = pd.concat([self.input_rest,df_input],ignore_index=False)
        df_input_converge.to_csv("static/res/model/temp_converge.csv",index=False)
        df_input_converge = pd.read_csv("static/res/model/temp_converge.csv")
        self.data_processed,_,_ =feature_engineering(df_input_converge)
